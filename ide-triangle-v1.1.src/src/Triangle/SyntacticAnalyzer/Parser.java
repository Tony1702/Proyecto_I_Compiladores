    ...
    case Token.SELECT:
      {
        ...
        Expression eAST = parseExpression();
        accept(Token.FROM);
        //Cases casAST = parseCases();
        accept(Token.END);
        finish(commandPos);
        //commandAST = new SelectCommand(eAST, casAST, commandPos);
      }
      break;
    ...
    case Token.LOOP:
      {
        ...
        switch(currentToken.kind){
            ...
            case Token.FOR:{
                accept(Token.FOR);
                Identifier iAST = parseIdentifier(); //Entero
                accept(Token.BECOMES);
                Expression e1AST = parseExpression(); //Entero
                accept(Token.TO);
                Expression e2AST = parseExpression(); //Entero
                accept(Token.DO);
                Command cAST = parseCommand();
                accept(Token.END);
                finish(commandPos);
    ...
    case Token.VAR:
      {
        ...
        else{ // <-- Alternativa Nueva
            accept(Token.BECOMES);
            Expression eAST = parseExpression();
            finish(declarationPos);
            declarationAST = new VarInitializationDeclaration(iAST, eAST, declarationPos);
        }    
      }
      break;
    ...
    case Token.PROC:
      {
        ...
        accept(Token.RPAREN);
        accept(Token.IS);
        Command cAST = parseCommand();  // <-- Nueva Linea
        accept(Token.END);              // <-- Nueva Linea
